...
/*enum for determine baudrate from 0 to 5 can make it macro declaration*/
typedef enum{
...
/* STRUCT to configuration USART TX(transmit bin) */
typedef struct{
...
/* STRUCT to configuration USART RX(received bin) */
...
/*Determine if any error happened can use all or bit by bit*/
typedef union{
	struct{
...
typedef struct{
    uint32 baudrate;
    baudrate_gen_t baudrate_gen_gonfig;   /*0-->5 */
